    ...
    const charger = async () => {
      ...
      await chargeur.lancer(sources, ({ id, valeur, erreur }) => {
        ...
        if (id === "coinmetrics") {
          ...
          setDonnees((d) => ({ ...d, mp }));
          publierQualiteChain(id, "Réseau BTC live", { sourceEffective: mp?.perime ? "cache mempool.space" : "mempool.space", observeLe: mp?.ts ?? null, recupereLe: mp?.ts ?? recupereLe, cadenceMs: 60_000, ageMaxMs: 5 * 60_000, couverture: mp ? { disponibles: 2, attendus: 2 } : null, estime: false, acces: mp === null ? "indisponible" : "public", statut: mp === null ? "indisponible" : mp.perime ? "perime" : "frais", ...(mp === null || erreur ? { raison: erreur ?? "mempool.space indisponible et aucun cache exploitable." } : {}) });
        } else if (id === "hashrate") {
          ...
          setDonnees((d) => ({ ...d, sol }));
          publierQualiteChain(id, "Réseau SOL", { sourceEffective: sol?.perime ? "cache RPC/CoinGecko" : "RPC PublicNode + CoinGecko", observeLe: sol?.ts ?? null, recupereLe: sol?.ts ?? recupereLe, cadenceMs: 60_000, ageMaxMs: 10 * 60_000, couverture: null, estime: false, acces: sol === null ? "indisponible" : "public", statut: sol === null ? "indisponible" : sol.perime ? "perime" : "frais", ...(erreur ? { raison: erreur } : {}) });
        }
      });
      if (ignore) return;
  ...
  return (
    <>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
        ...
        <Mineurs open={open} hashrate={donnees.hr} />
