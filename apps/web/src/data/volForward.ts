 ...
 * fenêtres de 7 j au plus : au-delà, l'excès de variance mêle la pente de la courbe.
 *
...
export interface SegmentVolForward {
  ...
  /** √max(σf² − σbase², 0) × √(dt/365), en % ; null sans événement, sans σ_base ou si dt > 7 j. */
...
/** Un segment par paire d'échéances consécutives (même ordre que `points`). */
export function volsForward(
  ...
  const courtes = (s: SegmentVolForward) => s.sigmaFwd !== null && s.dtJours <= DT_BASE_MAX_JOURS;
