/**
 * Liaison basse fréquence des UNITÉS DE TEMPS entre les slots de la grille multi-chart
 * (option persistée `syncTimeframe` de chart-layout). Module SÉPARÉ de chart-linking et
 * chargé à la demande par ChartGrid : il ne pèse pas sur le chargement initial (budget
 * JS d'entrée, mesuré au gzip près sur la CI GitHub).
 */
import { supportedTimeframesFor } from "../data/adapters";
import { chartLayoutStore, linkedTargets, visibleSlotCount } from "./chart-layout";
import { marketIdentity, marketStore, type MarketIdentity } from "./market";
