...
describe("WINDOW_REGISTRY", () => {
  it("contient exactement les 27 fenêtres attendues, sans doublon d'id ni de mnémonique", () => {
    expect(WINDOW_REGISTRY).toHaveLength(27);
    const ids = WINDOW_REGISTRY.map((w) => w.id);
    const mnemos = WINDOW_REGISTRY.map((w) => w.mnemonic);
    expect(new Set(ids).size).toBe(27);
    expect(new Set(mnemos).size).toBe(27);
