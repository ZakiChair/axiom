...
| Compteur | Clé localStorage **`axiom:cryptoquant:credits:v1`** (hors du préfixe d'archive `axiom:onchain:cq:`), valeur `{ "v": 1, "jours": { "AAAA-MM-JJ": crédits } }`, élaguée aux 31 derniers jours UTC à chaque écriture. Toute écriture est **additive sur la valeur stockée**, relue au passage ; la copie en mémoire du module est un **repli** en cas d'écriture refusée, **pas une source unique entre instances** (deux onglets partagent un seul `localStorage` et dépensent tous deux : la mémoire est réconciliée jour par jour par le maximum, et relue avant le contrôle de plafond). Illisible ou de version inconnue → traité comme vide, remplacé à la prochaine écriture. **Reste sur le poste** : ni exporté ni remplacé par l'import de sauvegarde (`persist.ts`, comme la clé) |
...
| 200 vide ou invalide | Facturé (compté) ; l'heure de la réponse est mémorisée en session par série et soumise à la même reprise 12 h (fin du nouvel appel à chaque ouverture) ; l'archive n'est pas modifiée. « Invalide » couvre le **corps illisible** (vide, tronqué, délai dépassé pendant la lecture) : la facturation et la mémoire sont solidaires. Seule exception : une **annulation par le consommateur** pendant la lecture reste « annulé » et ne pose aucune mémoire |
...
| Limites | Compteur **par navigateur** : les curl, un autre poste ou le déploiement Vercel ne sont pas vus ; le 402 reste le filet. Plusieurs onglets du MÊME navigateur sont bien couverts (écriture additive ci-dessus). La file de cadence (10 req/60 s), elle, reste **par instance** : deux onglets peuvent émettre jusqu'à 20 req/min et récolter des 429 (non facturés). Pas de lecture du solde réel (aucune API documentée) |
